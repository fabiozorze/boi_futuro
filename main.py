    ...
    while on == True:
        ...
        if (dt.hour >= 9) & (dt.minute >= 0):
            ...
            if (dt.minute == 0) & (get_diff != dt_hour_new):
                ...
                data_minute_new = exm.get_ohlc('BGI$', mt5.TIMEFRAME_M1, n=70000)
                ...
                print(pred.index[-1])
                logger.info(f'Old Row: {first_dollarbar.index[-1]} - New Row: {pred.index[-1]}')

                if pred.index[-1] != first_dollarbar.index[-1]:

                    try:
                        positions = mt5.positions_get(symbol='BGIK24')[0][9]
                    ...
                    logger.info(f'Old Pred: {pred.pred.iloc[-2]} - New Pred: {pred.pred.iloc[-1]}')
                    ...
                    if pred['pred'].iloc[-1] != pred['pred'].iloc[-2]:
                        ...
                        if positions == 0:
                            if (pred['pred'].iloc[-1] == 1):
                                ...
                                mt5.order_send(om.Sell(symbol='BGIK24',position_size=1))
            ...
            if (dt.hour >= 19) & (dt.minute >= 0):
                ...
                data_hour.to_csv(r'C:\Users\Fabio\PycharmProjects\boi_futuro\dados_save\data_hour.csv')
                data_dollar.to_csv(r'C:\Users\Fabio\PycharmProjects\boi_futuro\dados_save\data_dollar.csv')
                novos_dados_pred.to_csv(r'C:\Users\Fabio\PycharmProjects\boi_futuro\dados_save\novos_dados_pred.csv')

                print('break')
                logger.info('STOP STRATEGY - END OF THE DAY')
                break
